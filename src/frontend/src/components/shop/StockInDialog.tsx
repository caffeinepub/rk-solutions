...
export default function StockInDialog({ product, shopId, onClose }: StockInDialogProps) {
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="space-y-2">
            ...
            <p className="text-sm text-muted-foreground">
              Current stock: {Number(product.quantity)} units
            </p>
          </div>
          <DialogFooter>
            ...
            <Button type="submit" disabled={updateStock.isPending}>
              {updateStock.isPending ? 'Adding...' : 'Add Stock'}
            </Button>
          </DialogFooter>
        </form>
      </DialogContent>
